...
const Request = () => {
  ...
  const fetchSentRequests = async () => {
    try {
      const response = await axios.get('https://tutedude-ray7.onrender.com/friends/sent-requests', {
        headers: {
  ...
  const fetchReceivedRequests = async () => {
    try {
      const response = await axios.get('https://tutedude-ray7.onrender.com/friends/received-requests', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const manageRequest = async (friendRequestId, action) => {
    try {
      const response = await axios.post(
        'https://tutedude-ray7.onrender.com/friends/manage-request',
